...
# from gtrl_ros2.Environments.env_image_scout_hospital import GazeboEnv
...
def plot_animation_figure(ep):
    # 这里处理一下 ep 防止负索引
    ep_plot = max(0, ep - 200) # 原逻辑似乎是想画后面几轮? 或者这里是 offset? 保持原逻辑
    # 注意：如果 ep < 200，np.arange(ep-200) 会是空的。
    # 假设 ep 总是 >= 200
    ep_len = len(reward_target_list) # 使用列表实际长度更安全
    ...
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    plt.plot(np.arange(ep_len), reward_target_list)

    plt.subplot(2, 2, 2)
    plt.title('Collision Reward')
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    plt.plot(np.arange(ep_len), reward_collision_list)
    ...
    plt.plot(np.arange(ep_len), reward_list)
    plt.plot(np.arange(ep_len), reward_mean_list)

    plt.subplot(2, 2, 2)
    plt.title('Heuristic Reward')
    plt.xlabel('Episode')
    plt.ylabel('Heuristic Reward')
    plt.plot(np.arange(ep_len), reward_heuristic_list)

    plt.subplot(2, 2, 3)
    plt.title('Action Reward')
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    plt.plot(np.arange(ep_len), reward_action_list)

    plt.subplot(2, 2, 4)
    plt.title('Freeze Reward')
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    plt.plot(np.arange(ep_len), reward_freeze_list)
